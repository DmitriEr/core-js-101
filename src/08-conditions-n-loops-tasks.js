...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${bracketStart}${Math.min(a, b)}, ${Math.max(a, b)}${bracketEnd}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  const sum = arr.map((number, index, array) => {
    let element = number;
    if (array.length % 2 > 0) {
      if (index % 2 > 0) {
        element *= 2;
        if (element >= 10) {
          return element.toString().split('').reduce((a, b) => +a + +b, 0);
        }
      }
      return +element;
    }
...
function getDigitalRoot(num) {
  let digit = num;
  while (digit > 9) {
    digit = digit.toString().split('').reduce((prev, current) => +prev + +current, 0);
  }
  return digit;
}
...
function getCommonDirectoryPath(pathes) {
  if (pathes.filter((el) => el[0] === '/').length < pathes.length) {
    return '';
  }

  const [first, second] = pathes;
  return `${second.split('/').filter((element, index) => element === first.split('/')[index]).join('/')}/`;
}
